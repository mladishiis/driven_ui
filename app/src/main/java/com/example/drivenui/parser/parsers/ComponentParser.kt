...
class ComponentParser {
                ...
                when (eventType) {
                    XmlPullParser.START_TAG -> {
                        when (parser.name) {
                            ...
                            "property" -> parseProperty(parser)?.let { properties.add(it) }
    ...
    private fun parseWidgetComponent(parser: XmlPullParser): Component? {
        ...
        // Исправляем тип виджета: в XML иногда используется label вместо image
        val actualWidgetCode = when (widgetCode) {
            "label" -> {
                // Проверяем по bindingProperties или другим признакам
    ...
    // Вспомогательные методы парсинга остаются без изменений
