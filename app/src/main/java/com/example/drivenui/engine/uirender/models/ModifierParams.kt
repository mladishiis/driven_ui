...
data class ModifierParams(
    ...
    /**
     * Проценты по оси главного flex-контейнера задаёт [LayoutRenderer] (weight / fillMax*),
     * чтобы не дублировать sizing и не использовать fillMaxWidth(fraction) в Row.
     */
    private fun applyWidthModifier(modifier: Modifier): Modifier = when {
        widthFillFraction != null -> modifier
        width.equals("fillmax", ignoreCase = true) -> modifier.fillMaxWidth()
        width.equals("wrapcontent", ignoreCase = true) -> modifier.wrapContentWidth()
        else -> width.toIntOrNull()?.let { modifier.width(it.dp) } ?: modifier.fillMaxWidth()
    }

    private fun applyHeightModifier(modifier: Modifier): Modifier = when {
        heightFillFraction != null -> modifier
        height.equals("fillmax", ignoreCase = true) -> modifier.fillMaxHeight()
        height.equals("wrapcontent", ignoreCase = true) -> modifier.wrapContentHeight()
...
internal fun ComponentModel.sduiModifierParams(): ModifierParams = when (this) {
    is LayoutModel -> modifierParams
    is LabelModel -> modifierParams
    is ButtonModel -> modifierParams
    is ImageModel -> modifierParams
    is InputModel -> modifierParams
    is AppBarModel -> modifierParams
    else -> ModifierParams()
}
